...
def csv_into_df():
    def single_continent_csv_into_df(cities_list):
    ...
    us_pollution_dfs_dict = single_continent_csv_into_df(cities_us)
    eu_pollution_dfs_dict = single_continent_csv_into_df(cities_eu)

    return us_pollution_dfs_dict, eu_pollution_dfs_dict


def calculate_continent_avg_pollutions(us_pollution_dfs_dict, eu_pollution_dfs_dict):
    def single_continent_avg_pollutions(city_pollution_dfs_dict, cities):
    ...
    us_avgs = single_continent_avg_pollutions(us_pollution_dfs_dict, cities_us)
    eu_avgs = single_continent_avg_pollutions(eu_pollution_dfs_dict, cities_eu)

    return us_avgs, eu_avgs


def calculate_continent_median_pollutions(us_pollution_dfs_dict, eu_pollution_dfs_dict):
    def single_continent_median_pollutions(city_pollution_dfs_dict, cities):
        city_avgs = pd.DataFrame()
        for city in cities:
            mean_pollution = copy.deepcopy(city_pollution_dfs_dict[city])
            mean_pollution = mean_pollution.drop("date", axis=1)
    ...
    us_medians = single_continent_median_pollutions(us_pollution_dfs_dict, cities_us)
    eu_medians = single_continent_median_pollutions(eu_pollution_dfs_dict, cities_eu)

    return us_medians, eu_medians


def create_bar_chart(us_avgs, eu_avgs, type):
...
def calculate_city_avg_pollutions(us_pollution_dfs_dict, eu_pollution_dfs_dict):
    def single_continent_city_avg_pollutions(city_pollution_dfs_dict, cities):
        city_avgs = pd.DataFrame()
        for city in cities:
            mean_pollution = copy.deepcopy(city_pollution_dfs_dict[city])
            mean_pollution = mean_pollution.drop("date", axis=1)
    ...
    us_city_avgs = single_continent_city_avg_pollutions(us_pollution_dfs_dict, cities_us)
    eu_city_avgs = single_continent_city_avg_pollutions(eu_pollution_dfs_dict, cities_eu)

    return us_city_avgs, eu_city_avgs


def create_chart_compare_cities(us_city_avgs, eu_city_avgs):
